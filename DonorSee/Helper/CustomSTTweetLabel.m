...
@interface CustomSTTweetLabel () <UITextViewDelegate>
...
@property (nonatomic, strong) NSDictionary *attributesCustom;

@property (strong) UITextView *textView;

@end

@implementation CustomSTTweetLabel {
...
{
    [_textView sizeToFit];
    //    _textView.center = CGPointMake(_textView.center.x,
    //                                   self.bounds.size.height / 2.0f);
    CGSize szText = _textView.frame.size;
    self.frame = CGRectMake(self.frame.origin.x,
                            self.frame.origin.y,
                            _textView.frame.size.width,
                            _textView.frame.size.height);
    _textView.frame = CGRectMake(0, 0, szText.width, szText.height);
}
...
- (void)setupLabel {
    ...
    _customSelectionColor = [UIColor blackColor];
    
    _fontText = [UIFont fontWithName: FONT_LIGHT size: 12.0f];
    _fontUsername = [UIFont fontWithName: FONT_MEDIUM size: 12.0f];
...
{
    _attributesCustom = @{NSForegroundColorAttributeName: _customSelectionColor, NSFontAttributeName: _fontUsername};
    _attributesText = @{NSForegroundColorAttributeName: COLOR_FEED_TEXT,
                        NSFontAttributeName: _fontText};
    _attributesHandle = @{NSForegroundColorAttributeName: [UIColor redColor], NSFontAttributeName: _fontText};
    _attributesHashtag = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithWhite:170.0/255.0 alpha:1.0], NSFontAttributeName: _fontText};
    _attributesLink = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:129.0/255.0 green:171.0/255.0 blue:193.0/255.0 alpha:1.0], NSFontAttributeName: _fontText};
...
//                break;
        ...
        while (range.location + length < tmpText.length) {
            ...
            BOOL charIsMember = [validCharactersSet characterIsMember:[tmpText characterAtIndex:range.location + length]] | ([tmpText characterAtIndex:range.location + length] == [@"@" characterAtIndex: 0]);
        ...
        if (length > 1)
        {
            ...
            {
                [_rangesOfHotWords addObject:@{@"hotWord": @(hotWord), @"range": [NSValue valueWithRange:NSMakeRange(range.location - count, length - 1)]}];
                count ++;
            }
            else
            {
